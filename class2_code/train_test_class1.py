...
dataset = datasets.ImageFolder(r"/class2_code/train_data1", transform=transform)
...
def train(encoder, dataset, device='cpu', n_way=6, k_shot=5, q_query=3):
    ...
    for epoch in range(200):
        ...
        if loss < best_loss:
            best_loss = loss
            torch.save(encoder.state_dict(), r"/class2_code\best_class1.pth")
...
def predict_image(image_path, encoder_path=r"E:\实习工作项目2\oil\class1_code\best_class1.pth", class_names=None):
    ...
    for label_id, class_name in enumerate(class_names):
        class_dir = os.path.join(r'/class2_code\test数据', class_name)
