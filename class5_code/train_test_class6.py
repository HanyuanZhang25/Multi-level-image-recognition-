...
dataset = datasets.ImageFolder(r"/class5_code/class5_total_data/train_data6", transform=transform)
class_names = dataset.classes  # 获取类名
print("Classes:", class_names)  # 应为6类

# 3. 构造 few-shot episode,其中n_way取决于你的分类类别数量
...
# 5. 训练与保存最优模型
def train(encoder, dataset, device='cpu', n_way=4, k_shot=5, q_query=3):
    ...
    for epoch in range(200):
        ...
        if loss < best_loss:
            best_loss = loss
            torch.save(encoder.state_dict(), r"/class5_code/best_class6.pth")
            print(">> Best model saved.")

# 6. 测试函数：输入PNG图片，输出分类
def predict_image(image_path, encoder_path=r"E:\实习工作项目2\oil\class5_code\best_class5.pth", class_names=None):
    ...
    for label_id, class_name in enumerate(class_names):
        class_dir = os.path.join(r"/class5_code\class5_total_data\test_data", class_name)
...
if __name__ == '__main__':
    ...
    print(">> 开始训练...")
    train(encoder, dataset, device=device, n_way=4)
    """
    ...
    dir_path = r"E:\实习工作项目2\oil\class5_code\class5_total_data\test_data"
