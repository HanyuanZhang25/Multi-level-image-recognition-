...
dataset = datasets.ImageFolder(r"E:\实习工作项目2\oil\total_data\whole_get_data\train_dataset", transform=transform)
...
def train(encoder, dataset, device='cpu', n_way=6, k_shot=5, q_query=3):
    ...
    for epoch in range(200):
        ...
        if loss < best_loss:
            best_loss = loss
            torch.save(encoder.state_dict(), 'best_prototypical_encoder.pth')
            print(">> Best model saved.")

# 6. 测试函数：输入PNG图片，输出分类
def predict_image(image_path, encoder_path='best_prototypical_encoder.pth', class_names=None):
    ...
    for label_id, class_name in enumerate(class_names):
        class_dir = os.path.join(r"E:\实习工作项目2\oil\total_data\whole_get_data\test_dataset", class_name)
...
'''
    ...
    dir_path = r"E:\实习工作项目2\oil\total_data\whole_get_data\test_dataset"
